...
def handle_health() -> str:
    ...
    try:
        ...
        return f"Backend error: connection refused. Check that the services are running ({e.args[0] if e.args else 'unknown'})."
...
def handle_scores(lab_query: str = "") -> str:
    ...
    try:
        ...
        for rate in pass_rates:
            ...
            # avg_score is already a percentage (0-100), pass_rate would be 0-1
            pass_rate = rate.get("avg_score", rate.get("pass_rate", 0))
            # If pass_rate is < 1, it's likely a 0-1 scale, convert to percentage
            if pass_rate < 1:
